...
export default function AdminDashboard() {
  ...
  const [settingsForm, setSettingsForm] = useState({
    heroTitle: '', heroSubtitle: '', siteName: '', aboutUsText: ''
  });
  ...
  const approve = async (id) => {
    await axiosInstance.put(`/admin/tournaments/${id}/approve`);
    toast.success('Approved');
    fetchPending();
    fetchTournaments();
  };
  const reject = async (id) => {
    await axiosInstance.put(`/admin/tournaments/${id}/reject`);
  ...
  const handleSiteUpload = async (e, field) => {
    ...
    try {
      const res = await axiosInstance.post('/admin/settings/upload', fd);
      setSettings(prev => ({ ...prev, [field]: res.data[field] }));
  ...
  return (
    <div className="min-h-screen pt-24 pb-12 px-4 bg-gradient-to-b from-gray-950 to-black">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-extrabold bg-gradient-to-r from-cyan-400 to-purple-500 bg-clip-text text-transparent mb-8">
          Admin Dashboard
        </h1>
            ...
            {stats.monthlyData?.length > 0 && (
              <div className="glass-card p-6">
                <h3 className="text-xl font-semibold text-white mb-4">
                  <BarChart3 size={20} /> Registration Trends
                </h3>
          ...
          <div>
                ...
                {pending.map(t => (
                  <div key={t._id} className="flex justify-between items-center p-3 bg-white/5 rounded mb-2">
                    <div>
                      ...
                      <p className="text-gray-400 text-xs">{t.game} • by {t.createdBy?.name || 'User'}</p>
                    </div>
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-green-600 text-white px-5 py-2 rounded-full"
              >
                <Plus size={20} /> Create
              </button>
            </div>
                ...
                >
                  <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="md:col-span-2">
                      <input name="title" value={form.title} onChange={handleChange} required placeholder="Title *" className="input-modern" />
                    </div>
                    <div>
                      <input name="game" value={form.game} onChange={handleChange} placeholder="Game" className="input-modern" />
                    </div>
                    <div>
                      <input name="prize" value={form.prize} onChange={handleChange} placeholder="Prize" className="input-modern" />
                    </div>
                    <div>
                    ...
                    <div>
                      <input type="file" accept="image/*" onChange={(e) => setImageFile(e.target.files[0])} className="input-modern" />
                    </div>
                    <div className="md:col-span-2">
                      <textarea name="description" value={form.description} onChange={handleChange} rows={3} placeholder="Description" className="input-modern resize-none" />
                    </div>
                    <div className="md:col-span-2 flex justify-end gap-3">
                      <button type="button" onClick={resetForm} className="bg-gray-500 text-white px-6 py-2 rounded-full">Cancel</button>
                      <button type="submit" className="bg-blue-600 text-white px-6 py-2 rounded-full flex items-center gap-2">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-white/10">
                        <th className="text-left py-3 px-2 text-gray-400">Title</th>
                        <th className="text-left py-3 px-2 text-gray-400 hidden md:table-cell">Date</th>
                        <th className="text-left py-3 px-2 text-gray-400">Participants</th>
                    ...
                    <tbody>
                      {tournaments.map(t => (
                        <tr key={t._id} className="border-b border-white/5 hover:bg-white/5 transition">
                          ...
                          <td className="py-3 px-2">
                            <Users size={14} className="text-cyan-400 inline" /> {t.registeredUsers?.length || 0}/{t.maxParticipants}
                          </td>
                          <td className="py-3 px-2">
                            <div className="flex gap-2">
                              <button onClick={() => handleEdit(t)} className="p-1.5 rounded-full bg-blue-500/20 text-blue-400">
                                <Edit size={16} />
                              </button>
                              <button onClick={() => handleDelete(t._id)} className="p-1.5 rounded-full bg-red-500/20 text-red-400">
                                <Trash2 size={16} />
                              </button>
                              <Link to={`/tournaments/${t._id}`} className="p-1.5 rounded-full bg-green-500/20 text-green-400">
                                <Eye size={16} />
                              </Link>
                            </div>
                          </td>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="heroTitle" value={settingsForm.heroTitle} onChange={handleSettingsChange} className="input-modern" />
                </div>
                <div>
                  <label className="text-xs text-gray-400">Hero Subtitle</label>
                  <input name="heroSubtitle" value={settingsForm.heroSubtitle} onChange={handleSettingsChange} className="input-modern" />
                </div>
                <div>
                  <label className="text-xs text-gray-400">Site Name</label>
                  <input name="siteName" value={settingsForm.siteName} onChange={handleSettingsChange} className="input-modern" />
                </div>
              </div>
              <div className="mt-4">
                <label className="text-xs text-gray-400 block mb-1">About Us Text</label>
                <textarea
                  name="aboutUsText"
              ...
              <button
                ...
                className="mt-4 bg-cyan-600 hover:bg-cyan-700 text-white px-6 py-2 rounded-full flex items-center gap-2"
              >
                <Save size={16} /> Save All Text
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {['heroImage', 'feature1Image', 'feature2Image', 'feature3Image', 'feature4Image', 'logoUrl'].map(field => (
                  <div key={field} className="bg-white/5 p-4 rounded-xl">
                    ...
                    {settings[field] ? (
                      <img src={settings[field]} alt={field} className="w-full h-32 object-cover rounded-lg mb-2" />
                    ) : (
                    ...
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) => handleSiteUpload(e, field)}
