...
export default function AdminDashboard() {
  ...
  const [settingsForm, setSettingsForm] = useState({ heroTitle: '', heroSubtitle: '', siteName: '', aboutUsText: '' });
  ...
  const fetchSettings = async () => {
    try { const res = await axiosInstance.get('/admin/settings'); setSettings(res.data); setSettingsForm({ heroTitle: res.data.heroTitle || '', heroSubtitle: res.data.heroSubtitle || '', siteName: res.data.siteName || '', aboutUsText: res.data.aboutUsText || '' }); } catch (_) {}
  };
  ...
  const resetForm = () => { setShowForm(false); setEditingId(null); setForm({ title: '', description: '', game: 'Free Fire', date: '', registrationDeadline: '', maxParticipants: 100, prize: '', bannerImage: '' }); setImageFile(null); };
  ...
  const saveSettings = async () => { try { await axiosInstance.put('/admin/settings', settingsForm); toast.success('Settings saved'); } catch (_) { toast.error('Failed'); } };
        ...
        {activeTab === 'tournaments' && (
          <div>
              ...
              <div className="glass-card p-6 mb-8">
                ...
                {pending.map(t => (
                  <div key={t._id} className="flex justify-between items-center p-3 bg-white/5 rounded mb-2">
                    ...
                    <div className="flex gap-2"><button onClick={() => approve(t._id)} className="bg-green-600 text-white px-3 py-1 rounded">Approve</button><button onClick={() => reject(t._id)} className="bg-red-600 text-white px-3 py-1 rounded">Reject</button></div>
                  </div>
                ))}
              </div>
            )}
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => { resetForm(); setShowForm(true); }} className="flex items-center gap-2 bg-green-600 text-white px-5 py-2 rounded-full"><Plus size={20} /> Create</button>
            </div>
            <AnimatePresence>
              {showForm && (
          ...
          <div className="glass-card p-8 rounded-2xl space-y-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="text-xs text-gray-400">Hero Title</label><input name="heroTitle" value={settingsForm.heroTitle} onChange={handleSettingsChange} className="input-modern" /></div>
                <div><label className="text-xs text-gray-400">Hero Subtitle</label><input name="heroSubtitle" value={settingsForm.heroSubtitle} onChange={handleSettingsChange} className="input-modern" /></div>
                <div><label className="text-xs text-gray-400">Site Name</label><input name="siteName" value={settingsForm.siteName} onChange={handleSettingsChange} className="input-modern" /></div>
              </div>
              <div className="mt-4">
                <label className="text-xs text-gray-400 block mb-1">About Us Text</label>
                <textarea name="aboutUsText" value={settingsForm.aboutUsText || ''} onChange={handleSettingsChange} rows={6} className="input-modern resize-none" placeholder="Write about your platform…" />
              </div>
              <button onClick={saveSettings} className="mt-4 bg-cyan-600 hover:bg-cyan-700 text-white px-6 py-2 rounded-full flex items-center gap-2"><Save size={16} /> Save All Text</button>
            </div>
